...
def VectorFEJumpAvg(el1, el2, face, c, qorder):
	...
	for n in range(len(w)):
		...
		cn = coef@nor
		beta = cn@tau
		contra1 = 1/J1*vs1.T@F1.T@tau 
		contra2 = 1/J2*vs2.T@F2.T@tau 

		j = np.concatenate((beta*contra1, -beta*contra2))
		a = np.concatenate((s1, s2)) * (1 if face.boundary else .5) 
...
def R(x):
	...
	r = np.array([[1./3, offt/offb], [offt/offb, 1./3]])
	# r = np.array([[alpha, beta], [beta, alpha]])
	return rot@r@rot.T
	# return r
